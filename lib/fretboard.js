...
export function Fretboard(tuning) {
    ...
    const _this = {} // TODO: is this the best way to this?
    _this.tuning = tuning
    _this.fretboard = f
    _this.AllChords = []
    ...
    _this.Draw = (notes) => {
        ...
        if (notes.length < tuning.length) {
            ...
            chordConfigs.push(notesAux)
        } else if (notes.length === tunning.length) {
        ...
        for (let i = 0; i < chordConfigs.length; i++) {
            ...
            _this.buildChord(chordConfigs[i], 0, 0, [], 999, 0)
        }

        // TODO: sort by easiest to build
        // console.debug(_this.AllChords)
        return _this.AllChords
    }

    _this.buildChord = (remainingNotes, string, fret, currentResult, min, max) => {
        if (currentResult && (remainingNotes.length === 0 || string > _this.fretboard.length)) {
            _this.AllChords.push(currentResult)
        }
        const notesCp = [...remainingNotes]

        for (let i = string; i < _this.fretboard.length; i++) {
            for (let j = fret; j < NECK_SIZE; j++) {
                const noteIdx = notesCp.indexOf(_this.GetNote(i, j))
                if (noteIdx >= 0) {
                    ...
                    // console.debug(`Found the ${_this.fretboard[i][j]} string: ${i} fret: ${j}`)
                    const downStream = [...notesCp]
                    downStream.splice(noteIdx, 1)
                    currentResult[i] = j
                    _this.buildChord(downStream, i + 1, 0, [...currentResult], min, max)
                }
            }
        }
    }

    _this.GetNote = (string, fret) => {
        return _this.fretboard[string][fret]
    }
