...
describe("tonality lib", () => {
  ...
  it("normalizes root note", () => {
    let result = HarmonicMinorDiatonicScale("Am"); // should be smart enough to remove the 'm' from the root note
    let same_result = HarmonicMinorDiatonicScale("A");

    expect(result).toEqual(same_result);
  });
