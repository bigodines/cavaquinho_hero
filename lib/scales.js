...
// Generates the major scale given a root note (W W H W W W H)
export const Major = (root) => {
    const scaleSteps = [2, 2, 1, 2, 2, 2, 1]
    const degree = 1
    const lastAddedNote = root
    for (let i = 0; i < scaleSteps.length; i++) {
        // TODO: iterate, changing lastAddedNote according to scaleSteps using NoteAdd (like in chords)
    }
    return []
}
