...
/**
 * HarmonicMinorDiatonicScale generates the harmonic minor field of a given root note (also works with Im)
 * @param {string} root note
 * @param {boolean} tetrads if we should add the 7th to the resulting chord
 * @returns {Array} of diatonic chords
 */
...
export const PrepDim = (chord) => {
    const root = extractRootFromChord(chord)

    let target = NoteAdd(root, 11)
 ...
 * @param {string} chord
 * @returns {string} root note of the chord
 */
